...
let autoId = setInterval(nextSlide, 1500);
...
function showHideSections(linkId) {
   ...
   if (!currentSection.classList.contains(linkId)) {
      ...
      if (currentSection.classList.contains('home')) {
         // change
...
navLinks.forEach((link) =>
   link.addEventListener('click', () => {
      showHideSections(link.id);
   })
);

projectBtn.addEventListener('click', () => {
   showHideSections('projects');
});

contactsBtn.addEventListener('click', () => {
   showHideSections('contacts');
});

logoBtn.addEventListener('click', () => {
   showHideSections('home');
});
